...
export class ContractService {
	...
	private initialize() {
		if (!this.contract || !this.provider) {
			...
			this.provider = ethers.getDefaultProvider(process.env.NETWORK || 'sepolia', {})
			this.contract = new ethers.Contract(contractAddress, NFTMarketplaceABI, this.provider)
		}
	}
	...
	async verifyBidSignature(bid: Bid): Promise<boolean> {
		...
		try {
			...
			const recoveredAddress = ethers.verifyMessage(ethers.getBytes(hash), bid.signature)
