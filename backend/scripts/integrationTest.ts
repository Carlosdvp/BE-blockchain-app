	...
	try {
		...
		const results = {
			...
			listingVerified: listings.data.listings.some(
				(l: any) =>
					l.tokenId === signedListing.tokenId &&
					l.nftContract.toLowerCase() === signedListing.nftContract.toLowerCase()
			),
			bidVerified: bids.data.bids.some(
				(b: any) =>
					b.bidder.toLowerCase() === signedBid.bidder.toLowerCase() && b.amount === signedBid.amount
			)
		}
